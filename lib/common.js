...
module.exports = exports = {
  ...
  */
	getQueryDetails: function getQueryDetails(client, query) {
		var nullifiedQuery, viewName, tablesResult, isUpdatableResult, isUpdatable, tablesUsed, primaryKeys, primaryKeysResult;
		return _regeneratorRuntime.async(function getQueryDetails$(context$1$0) {
			while (1) switch (context$1$0.prev = context$1$0.next) {
				...
				case 13:
					...
					context$1$0.next = 18;
					return exports.performQuery(client, "SELECT a.attname\n\t\t\t\t\tFROM pg_index i\n\t\t\t\t\tJOIN pg_attribute a\n\t\t\t\t\t\tON a.attrelid = i.indrelid\n\t\t\t\t\t\t\tAND a.attnum = ANY(i.indkey)\n\t\t\t\t\tWHERE i.indrelid = $1::regclass\n\t\t\t\t\t\tAND i.indisprimary", [tablesUsed[0]]);

				case 18:
					primaryKeysResult = context$1$0.sent;

					primaryKeys = primaryKeysResult.rows.map(function (row) {
						return row.attname;
					});
  ...
  */
	getDiffFromSupplied: function getDiffFromSupplied(client, currentData, notifications, query, parsed, params) {
		var allRows, matched, oldHashes, newData, hasDelete, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, matchRow, cleanRow, _iteratorNormalCompletion2, _didIteratorError2, _iteratorError2, _iterator2, _step2, curIndex, sortProps, sortOrders, _iteratorNormalCompletion3, _didIteratorError3, _iteratorError3, _iterator3, _step3, index, diff;
			...
			while (1) switch (context$1$0.prev = context$1$0.next) {
				...
				case 6:
					...
					for (_iterator = _core.$for.getIterator(matched); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
						...
						if (!(parsed.fields.length === 1 && parsed.fields[0].constructor.name === "Star")) {
							(function () {

								var columnsSelected = {};
								parsed.fields.forEach(function (item) {
									// If the column is renamed in the output, rename the column
									columnsSelected[item.field.value] = item.name ? item.name.value : item.field.value;
								});
								...
								try {
									for (_iterator2 = _core.$for.getIterator(_core.Object.keys(columnsSelected)); !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
				...
				case 32:
					...
					context$1$0.prev = 38;
					// Fix indexes
					for (_iterator3 = _core.$for.getIterator(_.range(newData.length)); !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
						index = _step3.value;
				...
				case 42:
					context$1$0.prev = 42;
					context$1$0.t4 = context$1$0["catch"](38);
		...
		}, null, this, [[12, 16, 20, 28], [21,, 23, 27], [38, 42, 46, 54], [47,, 49, 53]]);
	},
