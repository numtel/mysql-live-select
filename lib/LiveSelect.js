      ...
      _this.triggers = _.map(triggers, function (columns, table) {
        return {
          ...
          validator: function (row) {
            ...
            _.forOwn(row, function (value, column) {
  ...
  _prototypeProperties(LiveSelect, null, {
    createView: {
      value: function createView(name, query, callback) {
        ...
        this.conn.query("CREATE OR REPLACE TEMP VIEW " + tmpName + " AS " + query, function (error, results) {
          _this.conn.query(tableUsageQuery, [tmpName], function (error, result) {
            ...
            _this.conn.query(columnUsageQuery, [tmpName], function (error, result) {
              if (error) return callback.call(_this, error);
        ...
        this.triggers.forEach(function (trigger) {
          trigger.handler.on("change", function (payload) {
            ...
            if (payload._op === "UPDATE") {
              trigger.columns.forEach(function (col) {
            ...
            var refresh = trigger.validator.call(_this, argVals);

            if (!_.isEmpty(refresh)) {
              _this.throttledRefresh(refresh);
            }
          });
      ...
      value: function update(rows, allIds) {
        ...
        rows.forEach(function (row) {
          ...
          if (_this.data[id]) {
            ...
            for (var col in _this.data[id].row) {
              if (_this.data[id].row[col] !== row[col]) {
                hasDiff = true;
                break;
              }
            }
