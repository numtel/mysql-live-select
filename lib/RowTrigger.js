  ...
  _prototypeProperties(RowTrigger, null, {
    stop: {
      value: function stop(callback) {
        ...
        var sql = ["DROP TRIGGER IF EXISTS \"" + this.triggerName + "\" ON " + this.table, "DROP FUNCTION IF EXISTS \"" + this.triggerName + "()\""];
...
function processQueue() {
  ...
  if (queue.length > 0) {
    ...
    querySequence(processItem.client, processItem.queries, function (error, results) {
