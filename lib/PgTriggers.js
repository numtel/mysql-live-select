	...
	_prototypeProperties(PgTriggers, null, {
		...
		refresh: {
			value: function refresh() {
				...
				var selectUpdates = this.waitingToUpdate.splice(0, updateCount).join("(),");

				return querySequence(this, ["SELECT " + selectUpdates + "()"]).then(function (results) {
					_.forOwn(results[0].rows[0], function (jsonData, updateFunction) {
						try {
							var data = JSON.parse(jsonData);
						} catch (error) {
							return _this.emit("error", error);
						}
						_this.emit(updateFunction, data);
						// TODO calc full results!
						_this.resultCache[updateFunction] = data;
					});
				}, function (error) {
					return _this.emit("error", error);
				});
		...
		updateResultCache: {
			value: function updateResultCache(updateFunction, diff) {},
			writable: true,
			configurable: true
		},
		getQueryTables: {
