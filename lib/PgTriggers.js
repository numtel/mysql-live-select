			...
			_this.getClient(function (error, client, done) {
				...
				client.on("notification", function (info) {
					if (info.channel === channel && info.payload in _this.triggerTables) {
						_this.triggerTables[info.payload].updateFunctions.map(function (updateFunction) {
							return querySequence(_this, ["SELECT " + updateFunction + "()"]).then(function (results) {
								_this.emit(updateFunction, results[0].rows);
								// TODO calc full results!
								_this.resultCache[updateFunction] = results[0].rows;
							});
						});
					}
				});
	...
	_prototypeProperties(PgTriggers, null, {
		...
		select: {
			value: function select(query, params) {
				...
				newSelect.init.then(function (result) {
					_this.registerQueryTriggers(newSelect.query, newSelect.updateFunction).then(function (tables) {
						newSelect.tablesUsed = tables;
					});
				});
				return newSelect;
			},
